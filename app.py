...
Analyze the resume against the job description honestly.
...
st.markdown("Upload your resume and paste a job description — get instant match score, skill gap analysis, and resume tips.")
st.markdown("---")
...
with col1:
    ...
    mode = st.radio("mode", ["📤 Upload PDF", "📋 Paste Text"],
        ...
        if pdf_file is not None:
            with st.spinner("Reading PDF..."):
                try:
                    ...
                    if resume_text:
                        st.success(f"✅ {len(resume_text.split())} words extracted")
                        with st.expander("Preview"):
                            st.text(resume_text[:500] + "...")
                    else:
                        st.error("No text found — PDF may be scanned. Use Paste Text instead.")
                except Exception as e:
                    st.error(f"PDF read failed: {e}")
    else:
        resume_text = st.text_area("Paste resume", placeholder="Paste your full resume here...",
                                    height=340, label_visibility="collapsed")

with col2:
    st.markdown("**📝 Job Description**")
    jd_text = st.text_area("Paste JD", placeholder="Paste the full job description here...",
...
if btn:
    ...
    with st.spinner("Analyzing... ~10 seconds"):
        try:
